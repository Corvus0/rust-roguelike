...
use super::{
    camera, gamelog::GameLog, CombatStats, Equipped, HungerClock, HungerState, InBackpack, Map,
...
fn draw_inventory(
    ...
    let mut j = 0;
    let source: Vec<(Entity, &Name)> = if equip {
...
pub fn show_inventory(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_inventory(gs, ctx, "Inventory".to_string(), false)
}

pub fn drop_item_menu(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_inventory(gs, ctx, "Drop Which Item?".to_string(), false)
}

pub fn remove_item_menu(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_inventory(gs, ctx, "Remove Which Item?".to_string(), true)
}
...
pub fn main_menu(gs: &mut State, ctx: &mut Rltk) -> MainMenuResult {
    ...
    {
        let selected_color = rltk::MAGENTA;
        let unselected_color = rltk::WHITE;
        let mut print_menu_item = |y, color, text| {
            ctx.print_color_centered(y, RGB::named(color), RGB::named(rltk::BLACK), text);
        };

        let mut newgame_color = unselected_color;
        let mut loadgame_color = unselected_color;
        let mut quit_color = unselected_color;

        let selected = match selection {
            MainMenuSelection::NewGame => &mut newgame_color,
            MainMenuSelection::LoadGame => &mut loadgame_color,
            MainMenuSelection::Quit => &mut quit_color,
        };

        *selected = selected_color;

        let mut y = 24;
        print_menu_item(y, newgame_color, "Begin New Game");
        y += 1;
        if save_exists {
            print_menu_item(25, loadgame_color, "Load Game");
            y += 1;
        }
        print_menu_item(y, quit_color, "Quit Game");
