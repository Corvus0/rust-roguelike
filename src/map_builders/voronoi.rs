...
impl VoronoiCellBuilder {
    fn new(distance_algorithm: DistanceAlgorithm) -> Self {
        Self {
            n_seeds: 64,
            distance_algorithm,
        }
    ...
    pub fn pythagoras() -> Box<Self> {
        Box::new(Self::new(DistanceAlgorithm::Pythagoras))
    }

    pub fn manhattan() -> Box<Self> {
        Box::new(Self::new(DistanceAlgorithm::Manhattan))
    }

    pub fn chebyshev() -> Box<Self> {
        Box::new(Self::new(DistanceAlgorithm::Chebyshev))
    }
    ...
    fn build(&mut self, rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        for (i, vid) in voronoi_membership.iter_mut().enumerate() {
            ...
            for (seed, pos) in voronoi_seeds.iter().enumerate() {
                let start = rltk::Point::new(x, y);
                let distance = match self.distance_algorithm {
                    DistanceAlgorithm::Pythagoras => {
                        rltk::DistanceAlg::PythagorasSquared.distance2d(start, pos.1)
                    }
                    DistanceAlgorithm::Manhattan => {
                        rltk::DistanceAlg::Manhattan.distance2d(start, pos.1)
                    }
                    DistanceAlgorithm::Chebyshev => {
                        rltk::DistanceAlg::Chebyshev.distance2d(start, pos.1)
                    }
                };
