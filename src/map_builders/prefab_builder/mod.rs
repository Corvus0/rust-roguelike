...
impl PrefabBuilder {
    fn new(mode: PrefabMode) -> Self {
        Self { mode: mode }
    }

    pub fn rex_level(template: &'static str) -> Box<Self> {
        Box::new(Self::new(PrefabMode::RexLevel { template }))
    }

    pub fn constant(level: prefab_levels::PrefabLevel) -> Box<Self> {
        Box::new(Self::new(PrefabMode::Constant { level }))
    }

    pub fn sectional(section: prefab_sections::PrefabSection) -> Box<Self> {
        Box::new(Self::new(PrefabMode::Sectional { section }))
    }

    pub fn vaults() -> Box<Self> {
        Box::new(Self::new(PrefabMode::RoomVaults))
    }
    ...
    fn apply_room_vaults(&mut self, build_data: &mut BuilderMap) {
        ...
        for _i in 0..n_vaults {
            ...
            loop {
                ...
                let y = (idx / build_data.map.height as usize) as i32;
